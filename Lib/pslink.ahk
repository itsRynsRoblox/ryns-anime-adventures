...
SavePSLinkSettings() {
    ...
    if !(PSLinkBox.Value = "" || (RegExMatch(PSLinkBox.Value, "^https://www\.roblox\.com/games/\d+.*"))) {
        MsgBox("Invalid Private Server Link! Read the guide to know how you can enter a valid link.", "Error", "+0x1000")
        PSLinkBox.Value := ""
        return
    }
...
HowToGetButton := PSLinkGUI.Add("Button", "x10 y60 w120 +BackgroundTrans", "PS Link Guide")
HowToGetButton.OnEvent("Click", (*) => PSLinkGuide())
